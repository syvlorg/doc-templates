{
    ...
    description = "A major mode for my documentation, with templates!";
    inputs = rec {
        settings.url = github:sylvorg/settings;
        titan.follows = "settings/titan";
        nixpkgs.follows = "settings/nixpkgs";

        epkg-titan-templates.url = github:syvlorg/titan-templates;
    ...
    outputs = inputs@{ self, flake-utils, settings, ... }: with builtins; with settings.lib; with flake-utils.lib; settings.mkOutputs {
        ...
        callPackage = { emacs, pname }: emacs.pkgs.trivialBuild rec {
            ...
            propagatedUserEnvPkgs = with emacs.pkgs; flatten [ titan-templates titan-templates.propagatedUserEnvPkgs ];
            meta = {
